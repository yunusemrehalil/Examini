...
class SolveQuestionFragment : BaseFragment(), View.OnClickListener {
    ...
    private fun setupObserver() {
        timerViewModel.timerLiveData.observe(viewLifecycleOwner) { millisUntilFinished ->
            timerOnTick(millisUntilFinished)
        }
        timerViewModel.timerFinishedLiveData.observe(viewLifecycleOwner) { isFinished ->
            if (isFinished) {
                timerOnFinish()
            }
        }
    ...
    private fun changeQuestion() {
        ...
        if (questionListWithoutCurrentQuestion.isNotEmpty()) {
            ...
            changeQuestionDialog.apply {
                setCancelable(false)
                setLevel(stringUtil.levelToString(changedQuestion.level))
                ...
                setChangeQuestionButtonListener {
                    ...
                    questionLevel = stringUtil.levelToString(changedQuestion.level)
